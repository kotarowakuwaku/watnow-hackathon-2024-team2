...
const Preview = ({ params }: { params: { favoriteName: string } }) => {
  ...
  const setFavorite = async (data: {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/oshi/save-oshi-info-and-genres`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const getGenles = async (data: { email: string }) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/genre/get-user-genres`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async () => {
    ...
    if (userEmail) {
      const submitData = {
        email: userEmail,
        oshi_name: getData.oshi_name,
        genre: selectedItems[0],
      };
      setSubmitting(true);
    ...
    <div className={styles.container}>
      {isLoading ? (
        ...
        <>
          ...
          {/* <img src={getData.image_url} alt={getData.oshi_name} className={styles.image} /> */}
          ...
          <div className={styles.titleContainer}>
            <a href={getData.official_site_url} className={styles.officialLink}>
              {getData.official_site_url}
            </a>
          </div>
          <div className={styles.titleContainer}>
